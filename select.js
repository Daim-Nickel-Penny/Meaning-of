...
const select = function () {
  ...
  if (selected !== "" && wordCount(selected) <= 2) {
    // only fire when string is not empty, and has word count less then 2
...
chrome.storage.onChanged.addListener(function (changes) {
  ...
  if (changes.anchor) {
    ...
    if (prevDict) {
      prevDict.classList.replace(
        changes.anchor.oldValue,
        changes.anchor.newValue
      );
    }
  }
});
